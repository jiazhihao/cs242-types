...
function replace(type, oldname, newtype){ // strips vars named oldname, adds in newtype
    if(type.isVar && type.name == oldname)
        return newtype;
    if(type.isVar && type.name != oldname)
        return Type.Var(oldname);
    if(type.isArrow)
...
function solveConstraints(todo) {
  ...
  while (c = todo.pop()) {
    ...
    if (lhs.isVar || rhs.isVar) {
        ...
        todo2 = [];
        while(c = todo.pop()){
            todo2.push([rename(c[0], oldname, newtype), rename(c[1], oldname, newtype)]);
        }
        todo = todo2;
    }
    else if (lhs.isArrow) {
        if (!rhs.isArrow)
            ...
            todo.append([lhs.from, rhs.from]);
            todo.append([lhs.to, rhs.to]);
        }
    }
    else if (lhs.isInt) {
        if (!rhs.isInt){
        ...
        if (!rhs.isPair)
            ...
            todo.append([lhs.fst, rhs.fst]);
            todo.append([lhs.snd, rhs.snd]);
        }
    }
  }
