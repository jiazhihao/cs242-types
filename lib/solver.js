...
Expr.prototype.typeOf = function(eq, ctx) {
  ...
  if (this.isApp) {
    ...
    var newTy = fresh();
    eq.push([funTy, Type.Arrow(argTy, newTy)]);
    return newTy;
  }
...
function solveConstraints(todo) {
  ...
  var solved = [];
  var c;
  // Using the magic template provided by assignment
  while (c = todo.pop()) {
    var lhs = c[0];
    var rhs = c[1];
    // if lhs or rhs is a variable, we update solved array
    // and may optionally perform substituions.
    if (lhs.isVar || rhs.isVar) {
      ...
      // If the newtype contains the old name of a variable, that
      // is considerred as an occurence bug
      if (!newtype.isVar && newtype.occurs(oldname))
      ...
      // we perform substition by renaming variables `oldname' with
      // new types 
      var todo2 = [];
      var c2;
      while(c2 = todo.pop()){
...
function inferTypes(decls) {
  ...
  for (var i = 0; i < tr.constraints.length; i++) {
    var lhs = tr.constraints[i][0];
    var rhs = tr.constraints[i][1];
    console.log(lhs.pretty(), " = ", rhs.pretty());
  }
